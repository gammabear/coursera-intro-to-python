...
def spawn_ball(direction):
    ...
    if direction == RIGHT:
        ball_vel = [VELX / 100.00, - VELY / 100.00]
...
def draw(canvas):
    ...
    # test if ball touches left or right gutters
    if ball_pos[0] <= BALL_RADIUS + PAD_WIDTH:
        
        if paddle1_pos - HALF_PAD_HEIGHT <= ball_pos[1] <= paddle1_pos + HALF_PAD_HEIGHT:
            # ball strikes left paddle
            ball_vel[0] = - ball_vel[0]
            
            # increase velocity of ball by %10
            ball_vel[0] *= 1.1
            ball_vel[1] *= 1.1
            
        else:
            # ball strikes left gutter
            score2 += 1 
            spawn_ball(RIGHT)
    ...
    # collide and reflect off top and bottom walls
    ...
    # draw scores
    canvas.draw_text(str(score1), [140,50], 40, "White")
    canvas.draw_text(str(score2), [440,50], 40, "White")
