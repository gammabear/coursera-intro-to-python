# implementation of Spaceship - program template RiceRocks
import simplegui
import math
import random
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated == True:
            canvas.draw_image(self.image, [self.image_center[0] + self.age % self.lifespan * self.image_size[0], self.image_center[1]], self.image_size,
                          self.pos, self.image_size, self.angle)
    ...
    def collide (self,other_object):
        ...
        if dist(p,q) <= self.radius + other_object.get_radius():
            return True
        else:
            return False
...
def draw(canvas):
    ...
    # handle rock ship collision
    if group_collide(rock_group, my_ship):
        lives -= 1
        
    # handle missile, rock collisions
...
def process_sprite_group(canvas, group):
    iter_group = set(group)
    
    for sprite in iter_group:
        sprite.draw(canvas)
        if sprite.update():
            group.remove(sprite) 
